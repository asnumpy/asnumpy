...
description: 当用户需要新增、修改、删除或排查 asnumpy 接口、CANN/NPU 算子、pybind11 绑定、Python 前端或 pytest 测试时使用。
---
...
- 简单一元算子优先用 `EXECUTE_UNARY_OP` 宏（`include/asnumpy/utils/acl_executor.hpp`），简单二元算子用 `EXECUTE_BINARY_OP`；两者会自动处理 workspace、同步和日志。
- `include/asnumpy/utils/npu_ops_macros.hpp` 中的 `DEFINE_UNARY_OP`/`DEFINE_BINARY_OP` 是直接生成完整函数定义的宏，与 `EXECUTE_*_OP` 用途不同，不要混用。
- 特殊 shape、返回多值、dtype 转换或多算子组合，参考 `csrc/linalg/`、`csrc/math/trigonometric_functions.cpp`（`Hypot`/`Radians`）中的手写实现；手写时遵循：`aclnnXxxGetWorkspaceSize(...)` → `AclWorkspace`（RAII）→ `aclnnXxx(...)` → `aclrtSynchronizeDevice()` → `ACLNN_CHECK`/`ACL_RT_CHECK`。
- C++ 后端日志使用 `spdlog`，通过宏 `LOG_DEBUG`、`LOG_INFO`、`LOG_WARN`（自动附加文件/函数前缀）、`ACL_DTYPE_WARN`（dtype 静默转换时警告）、`ACLNN_CHECK`、`ACL_RT_CHECK`（错误检查并抛出异常）；Python 前端日志使用 `loguru`（`from loguru import logger`）；不要在 C++ 侧引入 `printf` 或跨层混用日志库。
...
- Python 前端通常从 `._core.<module>` 相对导入绑定函数（如 `from ._core.math import sin as _sin`），做参数归一化后返回 `ndarray(...)`；dtype、shape、size 等转换优先复用 `src/asnumpy/utils.py` 的既有工具。
- 顶层 `asnumpy.xxx` 入口依赖 `src/asnumpy/__init__.py` 的懒加载映射；新增或删除公开 API 时必须同步检查映射、`TYPE_CHECKING` 导入和模块导出。
- 测试优先使用 `asnumpy.testing` 的装饰器，叠加顺序为：外层 `@testing.for_dtypes([dtype1, dtype2, ...])`，内层 `@testing.numpy_asnumpy_allclose(atol=..., rtol=...)` 或 `@testing.numpy_asnumpy_array_equal()`；测试函数签名为 `def test_xxx(xp, dtype)`，通过 `xp.ndarray.from_numpy(np_arr)` 创建 asnumpy 输入。

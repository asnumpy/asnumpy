...
def test_float8_e5m2_top_level_access():
    ...
    assert dtype_obj is not None, "ap.float8_e5m2 dtype 对象不应为空"
    # 验证 ap.float8_e5m2 是 np.dtype(ap.dtypes.float8_e5m2) 的结果
    assert ap.float8_e5m2 == np.dtype(ap.dtypes.float8_e5m2), "ap.float8_e5m2 应该等于 np.dtype(ap.dtypes.float8_e5m2)"
...
def test_float8_e5m2_as_numpy_dtype():
    ...
    # 测试直接使用 ap.float8_e5m2 创建数组（ap.float8_e5m2 已经是 dtype 对象）
    arr = np.array([1.0, 2.0, 3.14], dtype=ap.float8_e5m2)
    assert arr.dtype == ap.float8_e5m2, "数组 dtype 应该匹配"
    assert arr.dtype == np.dtype(ap.dtypes.float8_e5m2), "ap.float8_e5m2 和 np.dtype(ap.dtypes.float8_e5m2) 应该产生相同的 dtype"
    
    # 测试直接使用导入的 float8_e5m2 变量名（已经是 dtype 对象）
    from asnumpy import float8_e5m2
    assert float8_e5m2 == ap.float8_e5m2, "导入的 float8_e5m2 应该与 ap.float8_e5m2 相同"
    assert float8_e5m2 == arr.dtype, "float8_e5m2 应该与数组的 dtype 相同"
    logger.info(f"[PASS] ap.float8_e5m2 可以直接作为 numpy dtype 使用: {arr.dtype}")
    logger.info(f"[PASS] 导入的 float8_e5m2 可以直接使用: {float8_e5m2}")


def test_float8_e5m2_can_create_dtype():
    """检查 ap.float8_e5m2 是否已经是 numpy dtype 对象"""
    # ap.float8_e5m2 已经是 dtype 对象，可以直接使用
    assert ap.float8_e5m2 is not None, "ap.float8_e5m2 应该存在"
    assert isinstance(ap.float8_e5m2, np.dtype), "ap.float8_e5m2 应该是 numpy dtype 对象"
    logger.info(f"[PASS] ap.float8_e5m2 已经是 numpy dtype 对象: {ap.float8_e5m2}")


def test_float8_e5m2_can_create_array():
    """检查是否可以使用 float8_e5m2 创建数组"""
    arr = np.array([1.0, 2.0, 3.14], dtype=ap.float8_e5m2)
    assert arr.dtype == ap.float8_e5m2, "数组 dtype 应该匹配（ap.float8_e5m2 已经是 dtype 对象）"
...
def test_float8_e5m2_scalar_creation():
    ...
    # 测试创建标量（需要使用 ap.dtypes.float8_e5m2，因为 ap.float8_e5m2 是 dtype 对象）
    scalar = ap.dtypes.float8_e5m2(3.14)
...
def test_float8_e5m2_get_acl_enum():
    ...
    # 使用 ap.dtypes.float8_e5m2 创建标量（ap.float8_e5m2 是 dtype 对象，不能直接调用）
...
def test_float8_e5m2_ones_operator():
    """测试 ones 算子"""
    dtype = ap.float8_e5m2  # ap.float8_e5m2 已经是 dtype 对象
    expected_acl_value = 35  # ACL_FLOAT8_E5M2
...
def test_float8_e5m2_numpy_dtype_attributes():
    ...
    # 获取 dtype 对象（ap.float8_e5m2 已经是 dtype 对象）
    dtype = ap.float8_e5m2
    assert dtype is not None, "ap.float8_e5m2 应该存在"
    assert isinstance(dtype, np.dtype), "ap.float8_e5m2 应该是 numpy dtype 对象"
    ...
    dtype2 = ap.float8_e5m2  # ap.float8_e5m2 已经是 dtype 对象

...
def test_bfloat16_numpy_dtype_attributes():
    ...
    assert dtype_itemsize == 2, f"bfloat16 的 itemsize 应该是 2 字节，实际为 {dtype_itemsize}"
    ...
    try:
        ...
        if state is not None:
            ...
            new_dtype = ap.bfloat16  # ap.bfloat16 已经是 dtype 对象
...
def test_bfloat16_structured_and_subarray_types():
    ...
    # 创建包含 bfloat16 字段的结构化数组（使用 ap.dtypes.bfloat16 作为类型对象）
    structured_dtype = np.dtype([
        ('x', ap.dtypes.bfloat16),
        ('y', ap.dtypes.bfloat16),
        ('z', ap.dtypes.bfloat16)
    ])
    ...
    for name in structured_dtype.names:
        ...
        assert field_dtype == base_dtype, f"字段 '{name}' 的 dtype 应该是 bfloat16"
    ...
    # 创建 2x2 矩阵的子数组类型（使用 ap.dtypes.bfloat16 作为类型对象）
    subarray_dtype_2x2 = np.dtype((ap.dtypes.bfloat16, (2, 2)))
    ...
    assert subarray_dtype_2x2.subdtype[0] == base_dtype, "subdtype 的基础类型应该是 bfloat16"
    assert subarray_dtype_2x2.subdtype[1] == (2, 2), "subdtype 的形状应该是 (2, 2)"
    
    # 创建 1D 向量的子数组类型（使用 ap.dtypes.bfloat16 作为类型对象）
    subarray_dtype_1d = np.dtype((ap.dtypes.bfloat16, 3))
    ...
    assert subarray_dtype_1d.subdtype[0] == base_dtype, "subdtype 的基础类型应该是 bfloat16"
    ...
    # 创建包含子数组字段的结构化数组（使用 ap.dtypes.bfloat16 作为类型对象）
    complex_dtype = np.dtype([
        ('id', 'i4'),
        ('position', (ap.dtypes.bfloat16, 3)),      # 3D 位置向量
        ('matrix', (ap.dtypes.bfloat16, (2, 2)))    # 2x2 矩阵
    ])
    ...
    assert position_dtype.subdtype[0] == base_dtype, "position 的基础类型应该是 bfloat16"
    ...
    assert matrix_dtype.subdtype[0] == base_dtype, "matrix 的基础类型应该是 bfloat16"

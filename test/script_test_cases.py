...
# 双操作数测试数据
BINARY_TEST_CASES = [
    (np.array([-3, -2, -1, 0, 1, 2, 3], dtype=np.float32), np.array([0.5], dtype=np.float32)),
    (np.array([-1.5, 0.0, 2.7], dtype=np.float32), np.array([1.0], dtype=np.float32)),
    (np.array([0, 0, 0], dtype=np.float32), np.array([0.5], dtype=np.float32)),
    (np.array([1, 2, 3], dtype=np.float32), np.array([0.0], dtype=np.float32)),
    (np.array([-1, -2, -3], dtype=np.float32), np.array([1.0], dtype=np.float32)),
    (np.array([[0, 1], [-1, 0]], dtype=np.float32), np.array([[0.5, 0.5]], dtype=np.float32)),
    # 新增边缘测试用例
    (
    ...
    (
        ...
        np.array([1e-20, 1e20, 0], dtype=np.float32),
    ),  # 大数和小数
    (np.array([], dtype=np.float32), np.array([], dtype=np.float32)),  # 空数组 npuarray暂不支持
    (
        np.array([[1, 2], [3, 4]], dtype=np.float32),
...
BROADCAST_TEST_CASES = [
    ...
    (np.array([1, 2, 3], dtype=np.float32), np.array([[0.5], [1.0]], dtype=np.float32)),
    (np.array([0], dtype=np.float32), np.array([0.1, 0.2, 0.3], dtype=np.float32)),
    # 新增边缘广播用例
    (
        np.array([[np.nan], [0], [1]], dtype=np.float32),
        np.array([0.5, np.inf, -1.0], dtype=np.float32),
    ),
    (np.array([1e38, -1e38], dtype=np.float32), np.array([[0.0], [1.0]], dtype=np.float32)),
    (np.array([], dtype=np.float32), np.array([1.0, 2.0], dtype=np.float32)),  # 空数组广播
]
...
MATMUL_DOT_TEST_CASES = [
    ...
    # 向量点积 (1D x 1D)
    (np.array([1, 2, 9, 6], dtype=np.float32), np.array([4, 5, 10, 8], dtype=np.float32)),
    # 矩阵乘法 (2D x 2D)
    (np.array([[1, 2], [3, 4]], dtype=np.float32), np.array([[5, 6], [7, 8]], dtype=np.float32)),
    # 高维张量乘法 (3D x 3D)
    (
...
INNER_TEST_CASES = [
    ...
    # 1D vs 2D
    (np.array([1, 2], dtype=np.float32), np.array([[3, 4], [5, 6]], dtype=np.float32)),
    # 2D vs 1D
    (np.array([[1, 2], [3, 4]], dtype=np.float32), np.array([5, 6], dtype=np.float32)),
    # 2D vs 2D
    (np.array([[1, 2], [3, 4]], dtype=np.float32), np.array([[5, 6], [7, 8]], dtype=np.float32)),
    # 包含特殊值
    (np.array([0, np.nan, 1], dtype=np.float32), np.array([1, 0, np.inf], dtype=np.float32)),
    # 不同形状
    (np.array([1, 2, 3, 4], dtype=np.float32), np.array([5], dtype=np.float32)),
]
...
OUTER_TEST_CASES = [
    ...
    (np.array([[1, 2], [3, 4]], dtype=np.float32), np.array([5, 6], dtype=np.float32)),
    # 包含特殊值
    (np.array([0, np.nan], dtype=np.float32), np.array([1, np.inf], dtype=np.float32)),
    # 标量
...
NANSUM_DIM_TEST_CASES = [
    ...
    (
        np.array([[[[1.0, np.nan], [np.nan, 4.0]], [[5.0, 6.0], [7.0, np.nan]]]], dtype=np.float32),
        3,
        False,
    ),
]
...
CUM_TEST_CASES = [
    ...
    (
        np.array(
            [[[1.0, np.nan], [np.inf, 2.0]], [[-1.0, 0.0], [np.nan, -np.inf]]], dtype=np.float32
        ),
        2,
    ),
...
CROSS_TEST_CASES = [
    # 1D 向量测试用例
    (np.array([1.0, 2.0, 3.0], dtype=np.float32), np.array([4.0, 5.0, 6.0], dtype=np.float32), 0),
    (np.array([-1.0, 0.0, 1.0], dtype=np.float32), np.array([2.0, -2.0, 0.0], dtype=np.float32), 0),
    # 2D 数组测试用例 - 多个向量
    (
    ...
    # 正交向量测试
    (np.array([1.0, 0.0, 0.0], dtype=np.float32), np.array([0.0, 1.0, 0.0], dtype=np.float32), 0),
    # 平行向量测试（叉积为零）
    (
        np.array([-1.0, -2.0, -3.0], dtype=np.float32),
    ...
    # 零向量测试
    (np.array([0.0, 0.0, 0.0], dtype=np.float32), np.array([1.0, 2.0, 3.0], dtype=np.float32), 0),
]
...
GCD_TEST_CASES = [
    ...
    (
        np.array([-10, 15, -20], dtype=np.int64),
        np.array([-15, -25, 30], dtype=np.int64),
    ),  # 指定 int64
    # 相同数字场景
    ...
    (
        np.array([[[12, 18], [24, 30]]], dtype=np.int32),
        np.array([[[18, 24], [30, 36]]], dtype=np.int32),
    ),
    # 大整数
    (
        np.array([123456789, 987654321], dtype=np.int64),
        np.array([135792468, 864209753], dtype=np.int64),
    ),
    # 广播机制
...
CLIP_OBJ_OBJ_CASES = [
    ...
    (np.array([10.0, -10.0, 5.0], dtype=np.float32), 0.0, 6.0),
    (np.array([np.inf, -np.inf, np.nan, 3.3], dtype=np.float32), -5.0, 5.0),
    # float64
    (np.array([2.9, -2.9, 1e308, -1e308], dtype=np.float64), -10.0, 10.0),
    (np.array([np.inf, -np.inf, np.nan], dtype=np.float64), -1e5, 1e5),
]
...
NAN_TO_NUM_TEST_CASES = [
    ...
    # 2
    (np.array([0.0, -0.0, np.nan, np.inf], dtype=np.float32), 10.0, None, None),
    # 3
    (np.array([[np.nan, 100.0], [-np.inf, np.inf]], dtype=np.float32), -5.0, 100.0, -100.0),
]

MAX_MIN_TEST_CASES = [
    # 基础float32测试 - 相同形状
    (np.array([1.5, 2.7, -0.3], dtype=np.float32), np.array([2.0, 1.5, -0.1], dtype=np.float32)),
    # 广播测试 - 标量与数组
    (np.array([-1.5, 2.7, -0.3], dtype=np.float32), np.array(0.0, dtype=np.float32)),
    # 广播测试 - 不同维度
    (np.array([[1, 2], [3, 4]], dtype=np.float32), np.array([5, 6], dtype=np.float32)),
    # float64类型测试
    (
    ...
    # 正负零测试
    (np.array([0.0, -0.0, 0.0], dtype=np.float32), np.array([-0.0, 0.0, -0.0], dtype=np.float32)),
    # 极值测试
    (
    ...
    # 整数与浮点数混合
    (np.array([1, 2, 3], dtype=np.int32), np.array([2.5, 1.5, 3.5], dtype=np.float32)),
    # 重复值测试
    (np.array([5.0, 5.0, 5.0], dtype=np.float32), np.array([5.0, 5.0, 5.0], dtype=np.float32)),
    # 高维数组测试
    (
        np.array([[[1, 2], [3, 4]], [[5, 6], [7, 8]]], dtype=np.float32),
    ...
    # 使用默认dtype测试
    (np.array([-3.2, 4.7, -1.8], dtype=np.float32), np.array([-2.1, 3.9, -0.5], dtype=np.float32)),
]
...
LDEXP_TEST_CASES = [
    # === 基础测试 ===
    (np.array([1.0, 2.0, 3.0], dtype=np.float32), np.array([1, 2, 3], dtype=np.int32)),
    (np.array([-1.5, 0.0, 2.5], dtype=np.float32), np.array([-1, 0, 1], dtype=np.int32)),
    # === 广播测试 ===
    (
        np.array([[1.0, -2.0], [3.0, -4.0]], dtype=np.float32),
        np.array([1], dtype=np.int32),
    ),  # scalar-like broadcast
    (
        np.array([1.0, 2.0, 3.0], dtype=np.float32),
        np.array([[0], [1]], dtype=np.int32),
    ),  # 3 vs (2,1)
    # === 特殊数值 ===
    (np.array([np.inf, -np.inf, np.nan], dtype=np.float32), np.array([1, -1, 0], dtype=np.int32)),
    (
        np.array([0.0, -0.0, 1.0], dtype=np.float32),
        np.array([10, -10, 0], dtype=np.int32),
    ),  # 正负零处理
    # === 极大极小值 ===
    (np.array([1e-38, 1e-20, 1e20], dtype=np.float32), np.array([10, -10, 1], dtype=np.int32)),
    # === 高维张量 ===
    (np.array([[[1.0], [2.0]], [[3.0], [4.0]]], dtype=np.float32), np.array([1], dtype=np.int32)),
    # === 指数 float32（numpy 允许）===
    (np.array([1.0, 2.0, 3.0], dtype=np.float32), np.array([1.0, -1.0, 0.0], dtype=np.float32)),
]

COPYSIGN_TEST_CASES = [
    # === 基础测试 ===
    (np.array([1.0, -2.0, 3.0], dtype=np.float32), np.array([-1.0, 2.0, -3.0], dtype=np.float32)),
    (np.array([-1.5, 0.0, 2.5], dtype=np.float32), np.array([1.0, -1.0, 0.0], dtype=np.float32)),
    # === 广播测试 ===
    (
        np.array([[1.0, -2.0], [3.0, -4.0]], dtype=np.float32),
    ...
    # === 多维测试 ===
    (np.array([[1, -2], [-3, 4]], dtype=np.float32), np.array([[0, 1], [1, 0]], dtype=np.float32)),
    # === NaN 与广播混合 ===
    (
        np.array([[np.nan], [0], [1]], dtype=np.float32),
        np.array([0.5, np.inf, -1.0], dtype=np.float32),

    ...
    try:
        ...
        for value in test_values:
            try:
                ...
                print(
                    f"Input {value}({type(value).__name__}) -> Output {converted} (Match: {result})"
                )
            except Exception as e:
...
if __name__ == "__main__":
    ...
    boundary_tests = {
        np.float32: [0.0, -3.4028235e38, 3.4028235e38, 1.175494e-38],
        np.float64: [0.0, -1.7976931348623157e308, 1.7976931348623157e308, 2.2250738585072014e-308],
        np.int8: [0, -128, 127],
    ...
    for shape in shapes:
        try:
            ...
            print(
                f"Success! Shape: {arr_numpy.shape}, Value: {arr_numpy.ravel()[0] if arr_numpy.size > 0 else 'Empty array'}"
            )
            shape_success += 1
        except Exception as e:

    ...
    def __call__(self, *args, **kwargs):
        ...
        if op is None:
            ...
            if fallback is not None:
                return fallback(*processed_args, dtype=dtype)
            raise TypeError(
        ...
        if op.accepts_dtype and dtype is not None:
        ...
        # Write result to out buffer if specified
        if out is not None:
            out_targets = out if isinstance(out, tuple) else (out,)
            results = result_array if isinstance(result_array, tuple) else (result_array,)
            for dst, src in zip(out_targets, results, strict=False):
                dst[...] = src

...
def _get_max_array_dtype(args: list) -> np.dtype | None:
    """Return the highest-kind dtype among all array-like arguments."""
    ...
    if not array_dtypes:
        return None
    return max(array_dtypes, key=_dtype_kind)
...
class Op:
    ...
    __slots__ = ("in_types", "out_types", "routine")
...
def _parse_loop_table(loop_table, nin: int, nout: int) -> list[Op]:
    ...
    Args:
        loop_table: Either:
            - A single (type_sig_str, routine) pair, e.g. ('ff->f', _add)
            - A tuple of (type_sig_str, routine) pairs, e.g. (('ff->f', _add), ('dd->d', _add))
        nin: Number of inputs. If the type signature has more input chars than nin,
            only the first nin are used.
        nout: Number of outputs (used for validation).
    ...
    for entry in loop_table:
        if isinstance(entry, tuple) and len(entry) == 2:
            ...
            if isinstance(sig, str) and "->" in sig:
                ...
                # Truncate to nin inputs if signature has extra chars
                in_dtypes = in_dtypes[:nin]
                ops.append(Op(in_dtypes, out_dtypes, routine))
            else:
...
class Ops:
    ...
    Uses numpy.can_cast('same_kind') to match input dtypes against
    registered loop entries. First matching entry wins (same as NumPy ufunc).
    ...
    def guess_routine(
        ...
        """Find the best matching loop entry via can_cast, with caching.
    ...
    def __call__(self, *args, **kwargs):
            ...
            for i, arg in enumerate(processed_args):
                if _is_weak_scalar(arg):
                    ...
                    if _dtype_kind(scalar_dtype) <= _dtype_kind(max_array_dtype):
                        processed_args[i] = _coerce_weak_scalar(arg, max_array_dtype)
                        in_dtypes[i] = max_array_dtype
        ...
        if op is None:
            ...
            if fallback is not None:
                return fallback(*args, dtype=dtype)
            raise TypeError(
        ...
        try:
            if dtype is not None:
                ...
                # Try with dtype first, fall back to without
                try:
                    result = op.routine(*processed_args, dtype)
                except TypeError:
                    result = op.routine(*processed_args)
        except TypeError:
            # Routine may not accept dtype param (e.g. Sin, Sqrt)
            result = op.routine(*processed_args)
        ...
        if isinstance(result, _ndarray):
            ...
            return tuple(_ndarray(r) if not isinstance(r, _ndarray) else r for r in result)
        else:
...
def create_ufunc(
    ...
    first_entry = loop_table[0]
    if isinstance(first_entry, tuple) and len(first_entry) >= 2:
        first_sig = first_entry[0]
    else:
        raise ValueError(f"Invalid first entry in loop table for '{name}'")
    ...
    u = ufunc(name, Ops(ops_list, nin, nout), doc, default_casting)
    u._fallback = fallback
    return u

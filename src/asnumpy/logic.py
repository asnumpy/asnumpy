...
def _is_check_fallback(func, x, dtype=None):
    if dtype is not None:
        raise TypeError(f"{func.__name__}() does not support the 'dtype' parameter")
...
isfinite = _create_ufunc(
    ...
    fallback=lambda x, dtype=None: _is_check_fallback(_isfinite, x, dtype),

...
from ..lib.asnumpy_core import compiler as _lib
...
class PreparedKernel:
    ...
    def __init__(self, kernel: KernelFunction):
        ...
        self._start_event = _lib.create_event()
        self._end_event = _lib.create_event()
        self._stream = _lib.create_stream()
    ...
    def __call__(self, *args, stream=None) -> None:
        ...
        # Record start event
        _lib.record_event(self._start_event, self._stream)
        ...
        # Record end event
        _lib.record_event(self._end_event, self._stream)

        # Synchronize and measure
        _lib.synchronize_event(self._end_event)
        self._last_time_ms = _lib.elapsed_time_between(
            self._start_event, self._end_event
        )

    def __del__(self):
        try:
            _lib.destroy_event(self._start_event)
        except Exception:
            pass
        try:
            _lib.destroy_event(self._end_event)
        except Exception:
            pass
        try:
            _lib.destroy_stream(self._stream)
        except Exception:
            pass

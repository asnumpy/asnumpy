 ...
 private:
    ...
    static uint8_t encode_from_float(float f) {
        ...
        if (exp == constants::kFloat32MaxExponent) {
            ...
            return static_cast<uint8_t>((static_cast<uint8_t>(sign) << constants::kFloat6E2M3FnSignShift) | 0b0'11'111);
        }
        if (exp == 0 && frac == 0) {
            // 显式转换：从 uint32_t 到 uint8_t
            return static_cast<uint8_t>(static_cast<uint8_t>(sign) << constants::kFloat6E2M3FnSignShift);  // ±0
        }
        ...
        constexpr int bias = constants::kFloat6E2M3FnBias;  // E2M3 bias
        // 正规阈值：2^0 = 1
        if (exp == 0 || e_unbiased < constants::kFloat6E2M3FnSubnormalThreshold) {
            float mag = (exp == 0) ? mant : std::ldexp(mant, e_unbiased);
            int m = rne_to_int(static_cast<double>(mag) * constants::kFloat6E2M3FnSubnormalGrid);  // 3-bit mantissa
            if (m <= 0) {
                // 显式转换：从 uint32_t 到 uint8_t
        ...
        if (e > constants::kFloat6E2M3FnMaxExponent) {  // 最大指数 0b11 -> e_unbiased=2
            // 显式转换：从 uint32_t 到 uint8_t
            return static_cast<uint8_t>((static_cast<uint8_t>(sign) << constants::kFloat6E2M3FnSignShift) | 0b0'11'111);
        }
...
public:
    ...
    static constexpr int kMantissaBits = constants::kFloat6E2M3FnMantissaBits;
     ...
     explicit operator bool() const { return (rep_ & constants::kFloat6MantissaMask5) != 0; }

     float6_e2m3fn operator-() const { return FromRep(static_cast<uint8_t>(rep_ ^ constants::kFloat6SignBitMask)); }
 ...
 private:
     ...
     static uint8_t encode_from_float(float f) {
         ...
         if (exp == constants::kFloat32MaxExponent) {
             // 显式转换：从 uint32_t 到 uint8_t
             return static_cast<uint8_t>((static_cast<uint8_t>(sign) << constants::kFloat6E3M2FnSignShift) | 0b0'111'11);
         }
         if (exp == 0 && frac == 0) {
             // 显式转换：从 uint32_t 到 uint8_t
         ...
         constexpr int bias = constants::kFloat6E3M2FnBias;  // E3M2 bias
         // 正规阈值：2^-2 = 0.25
         if (exp == 0 || e_unbiased < constants::kFloat6E3M2FnSubnormalThreshold) {
            float mag = (exp == 0) ? mant : std::ldexp(mant, e_unbiased);
            int m = rne_to_int(static_cast<double>(mag) * constants::kFloat6E3M2FnSubnormalGrid);  // 2-bit mantissa
            if (m <= 0) {
                // 显式转换：从 uint32_t 到 uint8_t
             return static_cast<uint8_t>(static_cast<uint8_t>(sign) << constants::kFloat6E3M2FnSignShift);
            }
        ...
        if (e > constants::kFloat6E3M2FnMaxExponent) {  // 最大指数 0b111 -> e_unbiased=4
             // 显式转换：从 uint32_t 到 uint8_t
             return static_cast<uint8_t>((static_cast<uint8_t>(sign) << constants::kFloat6E3M2FnSignShift) | 0b0'111'11);
         }
 ...
 public:
     ...
     float6_e3m2fn operator-() const { return FromRep(static_cast<uint8_t>(rep_ ^ constants::kFloat6SignBitMask)); }

...
def run_test_case(
    shape: Tuple[int, ...], dtype: np.dtype = np.float32, warmup: int = 40, iterations: int = 400
) -> Dict[str, float]:
    """运行单个测试用例"""
    print(f"{'=' * 50}")
    print(f"测试形状: {shape}")
    ...
    try:
        # --- 测试 AsNumpy ---
        asnp_times = bench_exp2(ap.exp2, m1_asnp, warmup=warmup, iterations=iterations, is_npu=True)

        # --- 测试 NumPy ---
        np_times = bench_exp2(np.exp2, m1_np, warmup=warmup, iterations=iterations, is_npu=False)
